...
function begin_play() {
	...
	if (isEmpty(player_1) || isEmpty(player_2)) {
		...
		if(!started){
			...
			this.started = true;
			document.getElementById("turn_info").innerHTML="Player "+ whose_move() + " turn";
		}
...
function reset_play() {
	...
	for (var i = 0; i < this.table_ids.length; i++) {
...
function play()
 {
     if (this.started) {
         ...
         if (!this.table_ids.includes(move)) {
             alert("Invalid move.");
		 } 
		 else {
             if(this.board_state[this.table_ids.indexOf(move)] == -1)
             {
                 if (whose_move()) {
                     document.getElementById(move).innerHTML = "X";
                     this.board_state[this.table_ids.indexOf(move)] = 1;
                     document.getElementById("turn_info").innerHTML="Player "+ 2 + " turn";
                     toggle_move();
				 } 
				 else {
                     document.getElementById(move).innerHTML = "O";
                     this.board_state[this.table_ids.indexOf(move)] = 0;
                     document.getElementById("turn_info").innerHTML="Player "+ 1 + " turn";
                     toggle_move();
				 }
